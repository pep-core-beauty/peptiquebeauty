...
async function loadLiveStocks(){
  try{
    ...
    Object.entries(cart||{}).forEach(([oldKey,qty])=>{
      ...
      if(item && n>0){
        // Old + new cart keys can point to the same Sheet product after an update.
        // Keep the larger existing quantity instead of adding aliases together,
        // which prevents a 1-item cart from becoming 2 during migration.
        migratedCart[item.id]=Math.max(migratedCart[item.id]||0,n);
      }
    });
    cart=migratedCart;
...
let lastCartActionKey='';
let lastCartActionAt=0;
function cartActionAllowed(key){
  const now=Date.now();
  if(key===lastCartActionKey && now-lastCartActionAt<350) return false;
  lastCartActionKey=key;
...
document.addEventListener('click',e=>{
  ...
  if(variantAdd){
    ...
    if(!cartActionAllowed('add:'+key)) return;
  ...
  const add=e.target.closest('[data-add]'); if(add){const id=add.dataset.add;if(!cartActionAllowed('add:'+id))return;cart[id]=(cart[id]||0)+1;saveCart();toast('Added to your Peptique bag ♡');return}
  const qty=e.target.closest('[data-qty]'); if(qty){const id=qty.dataset.qty;if(!cartActionAllowed('qty:'+id+':'+qty.dataset.delta))return;cart[id]=(cart[id]||0)+Number(qty.dataset.delta);if(cart[id]<=0)delete cart[id];saveCart();return}
...
function normalizeShippingRegion(raw){
  ...
  if(!value)return '';
  if(value.includes('metro manila'))return 'Metro Manila';
  if(value.includes('visayas'))return 'Visayas';
  if(value.includes('mindanao'))return 'Mindanao';
  if(value.includes('luzon'))return 'Luzon';
  return '';
}
function selectedShippingRegion(){
  const byValue=normalizeShippingRegion(region?.value);
  if(byValue)return byValue;
  const optionText=region?.options?.[region.selectedIndex]?.textContent||'';
  return normalizeShippingRegion(optionText);
}
function recalcCheckout(){
  const sub=subtotal();const discount=packagingDiscount();let ship=0;let shipText='—';
  const deliveryMethod=String(delivery?.value||'').trim();
  const selectedRegion=selectedShippingRegion();
  if(deliveryMethod==='Lalamove'){
    shipText='Paid to rider';
    deliveryNote.hidden=false;
    deliveryNote.textContent='Lalamove delivery fee is paid directly to the rider upon delivery and is not included in your store total.';
    regionLabel.hidden=true;region.required=false;
  }else if(deliveryMethod.toUpperCase().includes('J&T')){
    regionLabel.hidden=false;region.required=true;
    deliveryNote.hidden=false;
    deliveryNote.textContent='J&T shipping is added to your order total.';
    if(selectedRegion){
      ship=Number(shippingRates[selectedRegion])||0;
      shipText=ship>0?peso(ship):'—';
    }
  }else{
    regionLabel.hidden=true;region.required=false;deliveryNote.hidden=true;
  }
  ...
  if(discountRow){
    ...
    if(discountValue)discountValue.textContent=discount>0?`−${peso(discount)}`:'—';
  }
  document.querySelector('#checkout-shipping').textContent=shipText;
  document.querySelector('#checkout-total').textContent=peso(sub-discount+ship);
  return {sub,discount,netSubtotal:sub-discount,ship,total:sub-discount+ship,shipText};
}
function handleDeliveryChange(){
  if(!String(delivery?.value||'').toUpperCase().includes('J&T'))region.value='';
  recalcCheckout();
}
...
document.querySelector('#checkout-form')?.addEventListener('change',recalcCheckout);
